...
import PhysicsGame, {ObjectPhysicsInfo, CollisionActionType, MoveDirType, CollisionCustom} from "../physicsGame"
import { Cube, Custom, getShape } from "./_shape"

export function checkCustomToCustom(objPhysInfo1: ObjectPhysicsInfo, objPhysInfo2: ObjectPhysicsInfo, physicsGame: PhysicsGame): boolean {
    let res = false
    res = res || _checkCustomToCustom(objPhysInfo1, objPhysInfo2, physicsGame)
    res = res || _checkCustomToCustom(objPhysInfo2, objPhysInfo1, physicsGame)
    return res
}

function _checkCustomToCustom(objPhysInfo1: ObjectPhysicsInfo, objPhysInfo2: ObjectPhysicsInfo, physicsGame: PhysicsGame): boolean {
    const obj1 = physicsGame.getObjectByPhysInfo(objPhysInfo1)
    const obj2 = physicsGame.getObjectByPhysInfo(objPhysInfo2)
    const shape1 = getShape(objPhysInfo1, physicsGame) as Custom
    const shape2 = getShape(objPhysInfo2, physicsGame) as Custom
...
export function actionBetweenCustomToCustom(actionType: CollisionActionType, objPhysInfo1: ObjectPhysicsInfo, objPhysInfo2: ObjectPhysicsInfo, moveDir: MoveDirType, physicsGame: PhysicsGame) {
    _actionBetweenCustomToCustom(actionType, objPhysInfo1, objPhysInfo2, moveDir, physicsGame)
    _actionBetweenCustomToCustom(actionType, objPhysInfo2, objPhysInfo1, moveDir, physicsGame)
}

function _actionBetweenCustomToCustom(actionType: CollisionActionType, objPhysInfo1: ObjectPhysicsInfo, objPhysInfo2: ObjectPhysicsInfo, moveDir: MoveDirType, physicsGame: PhysicsGame) {
    const POS_RATIO = physicsGame.settings.posRatio.cubeToCube
    const DELTA_TIME = physicsGame.settings.timeRatio.deltaTime

    const obj1 = physicsGame.getObjectByPhysInfo(objPhysInfo1)
    const obj2 = physicsGame.getObjectByPhysInfo(objPhysInfo2)
    const shape1 = getShape(objPhysInfo1, physicsGame) as Cube|Custom
    const shape2 = getShape(objPhysInfo2, physicsGame) as Cube|Custom
